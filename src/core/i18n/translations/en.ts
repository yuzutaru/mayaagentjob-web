export const en = {
  nav: {
    brandName: 'Maya',
    subtitle: 'Portfolio & Career Assistant',
    community: 'Community',
    findJobs: 'Find Jobs',
    companies: 'Companies',
    salaries: 'Salaries',
    portfolio: 'Portfolio',
    login: 'Login',
    signUp: 'Sign Up',
    logout: 'Logout',
    profile: 'Profile',
  },
  hero: {
    findYourDreamJob: 'Find Your Dream',
    job: 'Job',
    headline: 'Build a Portfolio Website & PDF CV',
    highlightWord: 'PDF CV',
    quote: '"Maya turns your skills into a stunning portfolio website and a shareable PDF CV — then finds the job that matches."',
    quoteHighlightWord: 'portfolio website',
    here: 'Here."',
    placeholderKeyword: 'Job title or keyword',
    placeholderCountry: 'Country or location',
  },
  categories: {
    title: 'Popular Categories',
    subtitle: 'Explore positions across high-growth domains',
    'cat-1': 'Programming and Tech',
    'cat-2': 'Graphics and Design',
    'cat-3': 'Digital Marketing',
    'cat-4': 'Video and animation',
    'cat-5': 'Business',
    'cat-6': 'Writing And Translation',
  },
  features: {
    title: 'One Platform, Everything You Need',
    subtitle: 'Build your portfolio website, export it as a PDF, and let AI match you to your next role.',
    learnMore: 'Learn More',
    'feature-portfolio': {
      title: 'Portfolio Web Builder',
      desc: 'Design a beautiful, responsive portfolio website from your real GitHub, GitLab or LinkedIn profiles.',
    },
    'feature-pdf': {
      title: 'PDF & CV Export',
      desc: 'Export a polished PDF CV and a shareable HTML site in one click — recruiter-ready in minutes.',
    },
    'feature-jobs': {
      title: 'AI Job Matching',
      desc: 'One of Maya\'s features: get roles matched to your technical stack and preferences.',
    },
  },
  jobs: {
    title: 'Find Your Dream Job',
    subtitle: 'Browse roles matched to your portfolio and preferences.',
  },
  vacancies: {
    title: 'Popular Vacancies',
    subtitle: 'Opportunities currently looking for talent',
    openPositions: 'Open Positions',
  },
  howItWorks: {
    title: 'How Maya Works',
    subtitle: 'Four simple steps to your next career milestone',
  },
  workSteps: {
    step1: {
      title: 'Import your profiles',
      desc: 'Connect GitHub, GitLab, Bitbucket or a LinkedIn PDF export to pull your work history.',
    },
    step2: {
      title: 'Build & customize',
      desc: 'Design your portfolio website with a live preview as you edit.',
    },
    step3: {
      title: 'Export PDF & site',
      desc: 'Download a polished PDF CV and your portfolio website in one click.',
    },
    step4: {
      title: 'Get AI matches',
      desc: 'Receive jobs matched to your portfolio and preferences.',
    },
  },
  cta: {
    candidate: {
      title: 'Build Your Portfolio',
      desc: 'Import your GitHub, GitLab or Bitbucket profile and generate a stunning portfolio & CV in one click.',
      btn: 'Open Builder',
    },
    jobs: {
      title: 'Find Your Dream Job',
      desc: 'Explore jobs matched to your technical stack and apply in seconds with Maya job search.',
      btn: 'Find Jobs',
    },
  },
  footer: {
    subtitle: 'Portfolio & Career Assistant',
    copyright: '© 2026 Maya. All rights reserved.',
    callNow: 'Call now',
    tagline: 'Build your portfolio website, export it as a PDF, and let AI match you to your next role.',
    columns: {
      'Quick Link': 'Quick Link',
      Candidate: 'Candidate',
      Employers: 'Employers',
      Support: 'Support',
    },
    links: {
      About: 'About',
      '→ Contact': '→ Contact',
      Pricing: 'Pricing',
      Blog: 'Blog',
      'Browse Jobs': 'Browse Jobs',
      'Browse Employers': 'Browse Employers',
      'Candidate Dashboard': 'Candidate Dashboard',
      'Saved Jobs': 'Saved Jobs',
      'Post a Job': 'Post a Job',
      'Browse Candidates': 'Browse Candidates',
      'Employers Dashboard': 'Employers Dashboard',
      Applications: 'Applications',
      Faqs: 'Faqs',
      'Privacy Policy': 'Privacy Policy',
      'Terms & Conditions': 'Terms & Conditions',
    },
  },
  jobListings: {
    sectionTitle: 'Available Jobs',
    found: 'found',
    emptyState: 'No jobs match your filters. Try adjusting your search or category.',
    matchScore: 'Match',
    applyNow: 'Apply Now',
  },
  onboarding: {
    joinMaya: 'Join Maya',
    tagline: 'Accelerate your job matching journey with AI',
    stepAuth: 'Auth',
    stepCvParse: 'CV Parse',
    stepCalibrate: 'Calibrate',
    continueGithub: 'Continue with GitHub',
    continueLinkedin: 'Continue with LinkedIn',
    continueGoogle: 'Continue with Google',
    or: 'or',
    signUpEmail: 'Sign Up with Email',
    parsingTitle: 'Parsing Profile Details...',
    parsingDesc: 'Maya is reading & converting {fileName} to match domain structures.',
    dragDropText: 'Drag & drop CV / Resume or LinkedIn PDF Export',
    dragDropSub: 'Supports PDF, DOCX up to 10MB',
    linkedinTip: 'LinkedIn Tip:',
    linkedinTipText: 'Export your profile page to PDF using LinkedIn\'s "More → Save to PDF" feature, and drag it here for instant import.',
    quickPicks: 'Simulator Quick-Picks:',
    softwareEngineerCv: '📄 Software Engineer CV',
    linkedinProductMgr: '🔵 LinkedIn PDF (Product Mgr)',
    skipManual: 'Skip and Fill Manually',
    aiParsedSuccess: 'AI Profile Parsed successfully!',
    aiParsedSuccessDesc: 'Review and refine your details below to ensure precise matching.',
    fullName: 'Full Name',
    emailAddress: 'Email Address',
    targetRoles: 'Target Roles',
    placeholderRoles: 'Add target role (e.g., Frontend Developer)',
    technicalStack: 'Technical Stack & Skills',
    placeholderTech: 'Add technology or skill (e.g., React)',
    workArrangements: 'Work Arrangements',
    targetLocations: 'Target Locations / Regions',
    placeholderLocations: 'Add location (e.g., Singapore)',
    aiProfileSummary: 'AI Profile Summary',
    startOver: 'Start Over',
    savingProfile: 'Saving profile...',
    saveComplete: 'Save & Complete Onboarding',
    completedTitle: 'All Set! Onboarding Complete',
    completedDesc: 'We\'ve successfully updated your candidate profile preferences. Redirecting you to your curated AI job matching feed...',
    letsFindJobs: 'Let\'s Find Jobs!',
  },
  tabs: {
    home: 'Home',
    saved: 'Saved',
    profile: 'Profile',
  },
  saved: {
    emptyTitle: 'No Saved Jobs Yet',
    emptySubtitle: 'Jobs you save will appear here for quick access.',
  },
  profile: {
    guest: 'Guest',
    notSignedIn: 'Not signed in',
    settings: 'Settings',
    darkMode: 'Dark Mode',
    language: 'Language',
    version: 'Version',
    on: 'On',
    off: 'Off',
    logout: 'Logout',
    loggingOut: 'Logging out...',
  },
  dashboard: {
    welcome: 'Welcome back',
    candidate: 'Candidate',
    subtitle: 'Here\'s your personalized job matching feed.',
    matchCount: 'AI Matches',
    profileScore: 'Profile Score',
    applications: 'Applications',
    recommendations: 'Recommendations',
    recommendationsDesc: 'Your personalized job recommendations based on your tech stack and preferences will appear here.',
  },
  portfolio: {
    title: 'Portfolio Builder',
    subtitle: 'Turn your dev profiles into a stunning portfolio & CV.',
    sourceHeading: '1. Choose a source',
    sourceSub: 'Pull data from your coding profiles, or start from scratch.',
    providers: {
      github: 'GitHub',
      gitlab: 'GitLab',
      bitbucket: 'Bitbucket',
      devto: 'Dev.to',
      stackoverflow: 'Stack Overflow',
      wakatime: 'WakaTime',
      linkedin: 'LinkedIn PDF',
    },
    startEmpty: 'Start Empty',
    startEmptyDesc: 'Build everything manually',
    loadSample: 'Load Sample',
    loadSampleDesc: 'Load a demo portfolio',
    usernameLabel: 'Username',
    usernamePlaceholder: 'e.g. alexrivera',
    apiKeyLabel: 'API key (optional)',
    apiKeyPlaceholder: 'Only for WakaTime',
    importBtn: 'Import',
    importing: 'Importing...',
    dropTitle: 'Drop a LinkedIn PDF export here',
    dropSub: 'or click to browse · PDF, TXT up to 10MB',
    editorHeading: '1. Customize your portfolio',
    editorSub: 'Everything you edit appears live in the preview.',
    basicInfo: 'Basic Info',
    fullName: 'Full Name',
    headline: 'Headline',
    bio: 'Bio',
    email: 'Email',
    phone: 'Phone',
    location: 'Location',
    website: 'Website',
    summary: 'Professional Summary',
    socials: 'Links & Socials',
    addSocial: 'Add link',
    socialPlatform: 'Platform',
    socialUrl: 'URL',
    socialHandle: 'Handle',
    skills: 'Skills',
    addSkill: 'Add skill',
    skillName: 'Skill',
    skillCategory: 'Category',
    experience: 'Experience',
    addExperience: 'Add experience',
    company: 'Company',
    role: 'Role',
    startDate: 'Start',
    endDate: 'End',
    present: 'Present',
    description: 'Description',
    projects: 'Projects',
    addProject: 'Add project',
    projectName: 'Project',
    techStack: 'Tech stack (comma separated)',
    language: 'Language',
    education: 'Education',
    addEducation: 'Add education',
    institution: 'Institution',
    degree: 'Degree',
    certifications: 'Certifications',
    addCertification: 'Add certification',
    certName: 'Certification',
    issuer: 'Issuer',
    year: 'Year',
    theme: 'Theme',
    accentColor: 'Accent color',
    previewHeading: 'Live Preview',
    previewSub: 'This is what your exported site looks like.',
    exportPdf: 'Download CV (PDF)',
    exportHtml: 'Download Site (HTML)',
    exporting: 'Exporting...',
    saveProfile: 'Save Portfolio',
    saving: 'Saving...',
    reset: 'Reset',
    remove: 'Remove',
    error: 'Something went wrong.',
    emptyHint: 'No items yet — add your first one above.',
    back: 'Back',
  },
  cvExport: {
    title: 'Export Your CV',
    subtitle: 'Pick an ATS-friendly or modern template, build your CV, then download it as a PDF.',
    chooseTemplate: '1. Choose a template',
    chooseSub: 'ATS-friendly layouts parse cleanly in applicant tracking systems.',
    atsSection: 'ATS-Friendly CV',
    atsSectionDesc: 'Simple, single-column layouts that parse reliably in applicant tracking systems.',
    modernSection: 'Modern / Creative',
    modernSectionDesc: 'Stand out with color and structure while staying recruiter-ready.',
    atsBadge: 'ATS',
    buildStep: '2. Build your CV',
    buildSub: 'Import from a profile or start from scratch. Your CV uses the selected template.',
    selectedTemplate: 'Selected template',
    changeTemplate: 'Change template',
    downloadCv: 'Download CV (PDF)',
    exporting: 'Exporting...',
  },
};
