export const id = {
  nav: {
    brandName: 'Maya',
    subtitle: 'Asisten Portofolio & Karir',
    community: 'Komunitas',
    findJobs: 'Cari Lowongan',
    companies: 'Perusahaan',
    salaries: 'Gaji',
    portfolio: 'Portofolio',
    login: 'Masuk',
    signUp: 'Daftar',
    logout: 'Keluar',
    profile: 'Profil',
  },
  hero: {
    findYourDreamJob: 'Temukan Pekerjaan Impian',
    job: 'Anda',
    headline: 'Bangun Website Portofolio & CV PDF',
    highlightWord: 'CV PDF',
    quote: '"Maya mengubah keahlian Anda menjadi website portofolio yang memukau dan CV PDF yang dapat dibagikan — lalu menemukan pekerjaan yang cocok."',
    quoteHighlightWord: 'website portofolio',
    here: 'Sini."',
    placeholderKeyword: 'Judul pekerjaan atau kata kunci',
    placeholderCountry: 'Negara atau lokasi',
  },
  categories: {
    title: 'Kategori Populer',
    subtitle: 'Jelajahi posisi di berbagai bidang yang berkembang pesat',
    'cat-1': 'Pemrograman & Teknologi',
    'cat-2': 'Desain & Grafis',
    'cat-3': 'Pemasaran Digital',
    'cat-4': 'Video & Animasi',
    'cat-5': 'Bisnis',
    'cat-6': 'Penulisan & Terjemahan',
  },
  features: {
    title: 'Satu Platform, Semua yang Anda Butuhkan',
    subtitle: 'Bangun website portofolio Anda, ekspor sebagai PDF, dan biarkan AI mencocokkan Anda dengan peran berikutnya.',
    learnMore: 'Pelajari Lebih Lanjut',
    'feature-portfolio': {
      title: 'Pembuat Website Portofolio',
      desc: 'Desain website portofolio yang indah dan responsif dari profil GitHub, GitLab, atau LinkedIn Anda yang sebenarnya.',
    },
    'feature-pdf': {
      title: 'Ekspor PDF & CV',
      desc: 'Ekspor CV PDF yang rapi dan situs HTML yang dapat dibagikan dalam satu klik — siap rekruter dalam hitungan menit.',
    },
    'feature-jobs': {
      title: 'Pencocokan Pekerjaan AI',
      desc: 'Salah satu fitur Maya: dapatkan peran yang cocok dengan keahlian dan preferensi Anda.',
    },
  },
  jobs: {
    title: 'Temukan Pekerjaan Impian',
    subtitle: 'Jelajahi peran yang cocok dengan portofolio dan preferensi Anda.',
  },
  vacancies: {
    title: 'Lowongan Populer',
    subtitle: 'Peluang kerja yang sedang mencari talenta',
    openPositions: 'Lowongan Aktif',
  },
  howItWorks: {
    title: 'Cara Kerja Maya',
    subtitle: 'Empat langkah mudah menuju jenjang karir Anda berikutnya',
  },
  workSteps: {
    step1: {
      title: 'Impor profil Anda',
      desc: 'Hubungkan GitHub, GitLab, Bitbucket atau ekspor PDF LinkedIn untuk menarik riwayat pekerjaan Anda.',
    },
    step2: {
      title: 'Bangun & sesuaikan',
      desc: 'Desain website portofolio Anda dengan pratinjau langsung saat mengedit.',
    },
    step3: {
      title: 'Ekspor PDF & situs',
      desc: 'Unduh CV PDF yang rapi dan website portofolio Anda dalam satu klik.',
    },
    step4: {
      title: 'Dapatkan kecocokan AI',
      desc: 'Terima pekerjaan yang cocok dengan portofolio dan preferensi Anda.',
    },
  },
  cta: {
    candidate: {
      title: 'Bangun Portofolio Anda',
      desc: 'Impor profil GitHub, GitLab, atau Bitbucket Anda dan buat portofolio & CV yang memukau dalam satu klik.',
      btn: 'Buka Pembuat',
    },
    jobs: {
      title: 'Temukan Pekerjaan Impian',
      desc: 'Jelajahi pekerjaan yang cocok dengan keahlian teknis Anda dan lamar dalam hitungan detik dengan pencarian kerja Maya.',
      btn: 'Cari Lowongan',
    },
  },
  footer: {
    subtitle: 'Asisten Portofolio & Karir',
    copyright: '© 2026 Maya. Hak Cipta Dilindungi Undang-Undang.',
    callNow: 'Hubungi sekarang',
    tagline: 'Bangun website portofolio Anda, ekspor sebagai PDF, dan biarkan AI mencocokkan Anda dengan peran berikutnya.',
    columns: {
      'Quick Link': 'Tautan Cepat',
      Candidate: 'Kandidat',
      Employers: 'Perusahaan',
      Support: 'Dukungan',
    },
    links: {
      About: 'Tentang',
      '→ Contact': '→ Kontak',
      Pricing: 'Harga',
      Blog: 'Blog',
      'Browse Jobs': 'Jelajahi Lowongan',
      'Browse Employers': 'Jelajahi Perusahaan',
      'Candidate Dashboard': 'Dasbor Kandidat',
      'Saved Jobs': 'Lowongan Tersimpan',
      'Post a Job': 'Pasang Lowongan',
      'Browse Candidates': 'Jelajahi Kandidat',
      'Employers Dashboard': 'Dasbor Perusahaan',
      Applications: 'Lamaran',
      Faqs: 'FAQ',
      'Privacy Policy': 'Kebijakan Privasi',
      'Terms & Conditions': 'Syarat & Ketentuan',
    },
  },
  jobListings: {
    sectionTitle: 'Lowongan Tersedia',
    found: 'ditemukan',
    emptyState: 'Tidak ada lowongan yang cocok dengan filter Anda. Coba sesuaikan pencarian atau kategori.',
    matchScore: 'Cocok',
    applyNow: 'Lamar Sekarang',
  },
  onboarding: {
    joinMaya: 'Bergabung dengan Maya',
    tagline: 'Percepat pencarian kerja Anda dengan AI',
    stepAuth: 'Autentikasi',
    stepCvParse: 'Analisis CV',
    stepCalibrate: 'Kalibrasi',
    continueGithub: 'Lanjutkan dengan GitHub',
    continueLinkedin: 'Lanjutkan dengan LinkedIn',
    continueGoogle: 'Lanjutkan dengan Google',
    or: 'atau',
    signUpEmail: 'Daftar dengan Email',
    parsingTitle: 'Menganalisis Detail Profil...',
    parsingDesc: 'Maya sedang membaca & mengonversi {fileName} untuk mencocokkan struktur bidang.',
    dragDropText: 'Tarik & lepas CV / Resume atau Ekspor PDF LinkedIn',
    dragDropSub: 'Mendukung PDF, DOCX hingga 10MB',
    linkedinTip: 'Tip LinkedIn:',
    linkedinTipText: 'Ekspor profil Anda ke PDF menggunakan fitur LinkedIn "Lainnya → Simpan ke PDF", lalu tarik ke sini untuk impor instan.',
    quickPicks: 'Pilihan Cepat Simulator:',
    softwareEngineerCv: '📄 CV Software Engineer',
    linkedinProductMgr: '🔵 PDF LinkedIn (Product Mgr)',
    skipManual: 'Lewati dan Isi Manual',
    aiParsedSuccess: 'Profil AI Berhasil Dianalisis!',
    aiParsedSuccessDesc: 'Tinjau dan sesuaikan detail Anda di bawah ini untuk pencocokan yang tepat.',
    fullName: 'Nama Lengkap',
    emailAddress: 'Alamat Email',
    targetRoles: 'Peran Target',
    placeholderRoles: 'Tambah peran target (misal, Frontend Developer)',
    technicalStack: 'Keahlian & Teknologi',
    placeholderTech: 'Tambah teknologi atau keahlian (misal, React)',
    workArrangements: 'Pengaturan Kerja',
    targetLocations: 'Lokasi / Wilayah Target',
    placeholderLocations: 'Tambah lokasi (misal, Singapura)',
    aiProfileSummary: 'Ringkasan Profil AI',
    startOver: 'Mulai Dari Awal',
    savingProfile: 'Menyimpan profil...',
    saveComplete: 'Simpan & Selesaikan Onboarding',
    completedTitle: 'Semua Selesai! Onboarding Selesai',
    completedDesc: 'Kami telah berhasil memperbarui preferensi profil kandidat Anda. Mengarahkan Anda ke daftar lowongan kerja pilihan AI Anda...',
    letsFindJobs: 'Ayo Cari Lowongan!',
  },
  tabs: {
    home: 'Beranda',
    saved: 'Tersimpan',
    profile: 'Profil',
  },
  saved: {
    emptyTitle: 'Belum Ada Lowongan Tersimpan',
    emptySubtitle: 'Lowongan yang Anda simpan akan muncul di sini untuk akses cepat.',
  },
  profile: {
    guest: 'Tamu',
    notSignedIn: 'Belum masuk',
    settings: 'Pengaturan',
    darkMode: 'Mode Gelap',
    language: 'Bahasa',
    version: 'Versi',
    on: 'Aktif',
    off: 'Nonaktif',
    logout: 'Keluar',
    loggingOut: 'Keluar...',
  },
  dashboard: {
    welcome: 'Selamat datang kembali',
    candidate: 'Kandidat',
    subtitle: 'Ini adalah feed pencocokan pekerjaan yang dipersonalisasi untuk Anda.',
    matchCount: 'Cocok AI',
    profileScore: 'Skor Profil',
    applications: 'Lamaran',
    recommendations: 'Rekomendasi',
    recommendationsDesc: 'Rekomendasi pekerjaan yang dipersonalisasi berdasarkan keahlian dan preferensi Anda akan muncul di sini.',
  },
  portfolio: {
    title: 'Pembuat Portofolio',
    subtitle: 'Ubah profil developer Anda menjadi portofolio & CV yang memukau.',
    sourceHeading: '1. Pilih sumber data',
    sourceSub: 'Ambil data dari profil coding Anda, atau mulai dari nol.',
    providers: {
      github: 'GitHub',
      gitlab: 'GitLab',
      bitbucket: 'Bitbucket',
      devto: 'Dev.to',
      stackoverflow: 'Stack Overflow',
      wakatime: 'WakaTime',
      linkedin: 'PDF LinkedIn',
    },
    startEmpty: 'Mulai Kosong',
    startEmptyDesc: 'Bangun semuanya secara manual',
    loadSample: 'Muat Contoh',
    loadSampleDesc: 'Muat portofolio demo',
    usernameLabel: 'Nama Pengguna',
    usernamePlaceholder: 'misal. alexrivera',
    apiKeyLabel: 'API key (opsional)',
    apiKeyPlaceholder: 'Khusus WakaTime',
    importBtn: 'Impor',
    importing: 'Mengimpor...',
    dropTitle: 'Letakkan ekspor PDF LinkedIn di sini',
    dropSub: 'atau klik untuk menjelajah · PDF, TXT hingga 10MB',
    editorHeading: '1. Sesuaikan portofolio Anda',
    editorSub: 'Semua yang Anda ubah tampil langsung di pratinjau.',
    basicInfo: 'Informasi Dasar',
    fullName: 'Nama Lengkap',
    headline: 'Jabatan',
    email: 'Email',
    phone: 'Telepon',
    location: 'Lokasi',
    website: 'Website',
    summary: 'Ringkasan Profesional',
    socials: 'Tautan & Sosial',
    addSocial: 'Tambah tautan',
    socialPlatform: 'Platform',
    socialUrl: 'URL',
    socialHandle: 'Nama Pengguna',
    skills: 'Keahlian',
    addSkill: 'Tambah keahlian',
    skillName: 'Keahlian',
    skillCategory: 'Kategori',
    experience: 'Pengalaman',
    addExperience: 'Tambah pengalaman',
    company: 'Perusahaan',
    role: 'Peran',
    startDate: 'Mulai',
    endDate: 'Selesai',
    present: 'Saat Ini',
    description: 'Deskripsi',
    projects: 'Proyek',
    addProject: 'Tambah proyek',
    projectName: 'Proyek',
    techStack: 'Tumpukan teknologi (pisahkan dengan koma)',
    language: 'Bahasa',
    education: 'Pendidikan',
    addEducation: 'Tambah pendidikan',
    institution: 'Institusi',
    degree: 'Gelar',
    certifications: 'Sertifikasi',
    addCertification: 'Tambah sertifikasi',
    certName: 'Sertifikasi',
    issuer: 'Penerbit',
    year: 'Tahun',
    theme: 'Tema',
    accentColor: 'Warna aksen',
    previewHeading: 'Pratinjau Langsung',
    previewSub: 'Inilah tampilan situs yang Anda ekspor.',
    exportPdf: 'Unduh CV (PDF)',
    exportHtml: 'Unduh Situs (HTML)',
    exporting: 'Mengekspor...',
    saveProfile: 'Simpan Portofolio',
    saving: 'Menyimpan...',
    reset: 'Reset',
    remove: 'Hapus',
    error: 'Terjadi kesalahan.',
    emptyHint: 'Belum ada item — tambahkan yang pertama di atas.',
    back: 'Kembali',
    letsConnect: 'Mari terhubung',
    builtWith: 'Dibangun dengan Maya',
    draftRestored: 'Draf yang belum disimpan telah dipulihkan. Setiap edit Anda tersimpan otomatis saat mengetik.',
    unsavedChanges: 'Ada perubahan yang belum disimpan. Tinggalkan halaman ini?',
  },
  cvExport: {
    title: 'Ekspor CV Anda',
    subtitle: 'Pilih templat ramah-ATS atau modern, susun CV Anda, lalu unduh sebagai PDF.',
    chooseTemplate: '1. Pilih templat',
    chooseSub: 'Templat ramah-ATS terbaca rapi di sistem pelacak pelamar.',
    atsSection: 'CV Ramah-ATS',
    atsSectionDesc: 'Tata letak satu kolom sederhana yang terurai andal di sistem pelacak pelamar.',
    modernSection: 'Modern / Kreatif',
    modernSectionDesc: 'Menonjol dengan warna dan struktur namun tetap siap rekruter.',
    atsBadge: 'ATS',
    buildStep: '2. Susun CV Anda',
    buildSub: 'Impor dari profil atau mulai dari awal. CV Anda memakai templat terpilih.',
    selectedTemplate: 'Templat terpilih',
    changeTemplate: 'Ganti templat',
    downloadCv: 'Unduh CV (PDF)',
    exporting: 'Mengekspor...',
  },
};
