...
2. **Review Domain Types**: Inspect files in `src/domain/` to verify that any data mapping aligns with domain types.
3. **Reference Global Rules**: Consult `.agents/AGENTS.md` for multi-repo workspace invariants.
...
3. **Strict Layer Isolation**:
   - UI views must never execute raw PostgREST queries or fetch queries directly.
   - All network and Supabase transactions must reside in `src/data/repositories/` following interfaces defined in `src/domain/repositories/`.
...
## ⚡ Tooling: RTK Usage
Ensure all CLI scripts and git actions utilize RTK (Rust Token Killer):
- `rtk gain` — check token usage history and cumulative prompt optimization stats.
